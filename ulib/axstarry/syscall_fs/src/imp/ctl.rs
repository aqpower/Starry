...
pub fn syscall_renameat2(
    ...
    let old_metadata = match metadata(old_path.path()) {
    ...
    //         return Err(SyscallError::ENOTDIR); // Cannot replace a directory with a file
    //     }
    ...
    // } else {
    // return Err(SyscallError::EINVAL); // Invalid argument
    // }
